...
def u_login(request):
    ...
    if request.method=='POST':
        ...
        if user is not None:
            if user.is_active and user.is_superuser != True:
                login(request, user)
                resp['status']='success'
                ...
                if user.is_superuser != True:
                    ...
                    resp['msg'] = "get access from the admin side"
                else:
        ...
        if user is not None:
            ...
            if user.is_active and user.is_superuser == True:
                ...
                if user.is_superuser != True:
                    ...
                    resp['msg'] = "Your are not authorised to access the admin side"
                else:
                    resp['msg'] = "Incorrect username or password"
        else:
            resp['msg'] = "Incorrect username or password"
...
def signup(request):
    if request.method=='POST':
        ...
        if passw != cpassw:
            messages.error(request,"Password And Confrim Password did'nt match")
            return redirect('/signup')
        else:
            user = User.objects.create(username=email,email=email,password=passw)
